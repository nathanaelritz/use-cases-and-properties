...
* Intermediary Mediation and TLS Termination: Some deployments route traffic
  through intermediaries such as reverse proxies, gateways, WAFs, service-mesh
  components, and CDN edges. The protocol and local policy should make clear
  which of two distinct scenarios applies: (a) the intermediary is itself the
  attested component, in which case attestation semantics apply to the
  intermediary's TEE and the trust boundary ends there; or (b) the intermediary
  terminates TLS on behalf of an origin that the endpoint wishes to attest
  independently, in which case the architecture must address how channel
  binding to the origin is established or preserved across the intermediary.
  Some relying parties will view a TLS-terminating intermediary as an attack,
  even when it is a load balancer or reverse proxy. At the same time, authorized
  and cooperative intermediaries can be structurally identical from the
  protocol's perspective. For example, an enterprise WAF operating under an
  MDM-installed corporate root can sit in the same protocol position as a
  hostile MitM, although the deployment intent is different. The solution should
  therefore allow relying parties to distinguish authorized and cooperative
  intermediary deployments from adversarial or unexpected intermediaries that
  occupy the same protocol position. The solution should also support detection
  of cases where a previously direct TLS connection is replaced with a
  terminating or load-balancing proxy, so that such a topology change can be
  identified and handled according to policy.
